...
function getRectLayout(){
    ...
    rect.computer = getComputer(rect.child , rect.grandChild , computerData);
    rect.logo = transLogo;
...
//TODO:1.多个区域的布局未处理  2.tooltip的数据展示未处理  3.未测试没有被画入的区域屏蔽攻击效果
// TODO：4.攻击点的坐标 有点偏，需要稍微调整  5.根据攻击类型用不同的颜色划线和点(接口加字段)  6.如何表示外界攻击的点坐标（接口协商如何表示）  7,ajax读取数据后的处理
...
function transOriData(grandChildData , computerData , childRectData , rectData){
    ...
    for (var j=0; j<rectData.length ; j++){
        ...
        //匹配图片
        if (rectData[j].img == "1.png"){
            rectData[j].img = "img/oa.png";
        }else if(rectData[j].img == "2.png"){
            rectData[j].img = "img/test.png";
        }else if(rectData[j].img == "3.png") {
        ...
        //TODO：修改变量名
        rectObj.rectName = rectData[j].rectName;
...
//计算父区域布局
function calculate(obj,rect , childRectData , grandChildData) {
    ...
    for(var m=0; m<childRectData.length ; m++){
        ...
        if((rect.id == childRectData[m].pid) && (childRectData[m].height>0)) {
            ...
            childRect.rectName = childRectData[m].rectName;
            ...
            if(childRectData[m].hasGrandChild){
                for (var j=0 ; j<grandChildData.length ; j++){
                    ...
                    grandChildRect.rectName = grandChildData[j].rectName;
                    grandNum ++;
            ...
            logoRect.y = rect.y + rect.height*0.4;
...
function getRectAttack(rect){
    ...
    for(var i=0 ; i<rect.length ; i++) {
        ...
        att.y = rect[i].y + rect[i].height/2;
